...
# --- 3. CONEXÃO E CACHE (SOLUÇÃO PARA ERRO 429) ---
@st.cache_resource
def init_connection():
    try:
...
@st.cache_data(ttl=60) # Cache de 1 minuto para não estourar a cota
...
def calcular_taxa_reserva(row):
    ...
    # REGRA: 13% se for Booking. 0% se for Telefone/Whats. + Taxa da Maquininha.
...
def update_row(sheet_name, row_id, new_data):
    ...
    for i, row in enumerate(data):
        if str(row.get('id')) == str(row_id):
            # Range dinâmico baseado no tamanho dos dados
...
if menu == "💰 Dashboard":
        ...
        with st.container(border=True):
            ...
            with st.form("form_r"):
                ...
                if st.form_submit_button("✅ SALVAR"):
                    ...
                    if mode=="editar": update_row("reservas", rid, new)
                    else: 
    ...
    if not df_r.empty:
        ...
        labels = ["ID", "Hóspede", "Entrada", "Quarto", "Total", "Pgto", "📝", "🗑️"]
        ...
        with st.container(border=True):
            ...
            with st.form("form_d"):
                ...
                if st.form_submit_button("✅ SALVAR"):
                    ...
                    if st.session_state.edit_mode_d == "editar": update_row("despesas", did, new_d)
                    else: 
