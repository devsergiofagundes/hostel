...
def seletor_periodo():
    ...
    with c3:
        # CORREÇÃO AQUI: pd.DateOffset em vez de pd.Offset
...
if menu == "💰 Dashboard":
    ...
    if not df_r.empty:
        ...
        taxas = bruto * 0.18 # 13% site + 5% gateway
    ...
    c2.metric("TAXAS (18%)", f"R$ {taxas:,.2f}")
    c3.metric("OPERACIONAL", f"R$ {operacionais:,.2f}")
    c4.metric("LUCRO REAL", f"R$ {(bruto - taxas - operacionais):,.2f}")

# --- RESERVAS (APENAS MÊS SELECIONADO) ---
elif menu == "📋 Reservas":
...
# --- DESPESAS (APENAS MÊS SELECIONADO) ---
elif menu == "💸 Despesas":
